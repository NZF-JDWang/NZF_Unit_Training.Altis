							...
							if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
								...
								if (_aMin > 0) then {
										...
										if (!surfaceiswater _mPos) then {
											...
											[_aGroup, _mPos, 500,[],true,false,-1] call lambs_wp_fnc_taskGarrison;
												}else{
												//0 = [_aGroup,_mkr] call EOS_fnc_taskpatrol;
												[_aGroup, _mPos, 250, 6, [], true] call lambs_wp_fnc_taskPatrol;
													};
				...
				_units=_eosActivated getvariable _cacheGrp;	
					...
					if (_debug)then{player sidechat format ["ID:%1,restore - %2",_cacheGrp,_units];};
							...
							if (_bMin > 0) then {	
									...
									_bGroup setVariable ["lambs_danger_enableGroupReinforce", true, true];
										//0 = [_bGroup,_mkr] call EOS_fnc_taskpatrol;
										if (random 100 > 25) then 
										{
											[_bGroup, _mPos, 250, 8, [], true] call lambs_wp_fnc_taskPatrol;
										} else 
											{
												[_bGroup, 500] spawn lambs_wp_fnc_taskCreep;
											};
				...
				if ((_cSize select 0) > 0) then{
						0=[(_cGroup select 0),_cSize,(_cGroup select 2),_faction,_cargoType] call eos_fnc_setcargo;
							...
							0=[(_cGroup select 2),"LIGskill"] call eos_fnc_grouphandlers;
								...
								_cGroup#2 setBehaviour "SAFE";
								[_cGroup#2, _mPos, 500, 5, [], true] call lambs_wp_fnc_taskPatrol;
...
//SPAWN STATIC PLACEMENTS
	for "_counter" from 1 to _eGrps do {
		...
		//_newpos=[_mPos,50] call EOS_fnc_findSafePos;
		_newpos=[_mPos,25,75,0,0,10] call BIS_fnc_findSafePos;
